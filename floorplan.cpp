...
bool backtrack(InputMapType& input, OutputMapType& output, vector<vector<bool> >& grid, InputMapType::iterator it)
{
    //base
    if (it == input.end()){
        return true;
    ...
    for (unsigned int i = 0; i < grid.size(); i++){
        for (unsigned int j = 0; j < grid[i].size(); j++){
            ...
            if (isValid(i , j, it->second, grid)){
                flip(i, j, it->second, grid);
                //if next rectangle returns true; insert into the output map
                if (backtrack(input, output, grid, ++temp)){
                    output.insert(make_pair(it->first, pair<int, int>(i,j)));
                    return true;
                }
                else{
                    flip(i, j, it->second, grid);
